package com.duhos.duhosii.calendar;
...
import com.duhos.duhosii.models.AlarmDate;
import com.duhos.duhosii.models.Event;
import com.duhos.duhosii.utils.ReminderBroadcast;
...
public class CalendarItemAdapter extends RecyclerView.Adapter<CalendarItemAdapter.ViewHolder> {
    ...
    private List<AlarmDate> konacnaListaAlarma;
    ...
    @Override
    public void onBindViewHolder(@NonNull final CalendarItemAdapter.ViewHolder holder, final int position) {
        ...
        if(visible){
            ...
            for(int i=0;i<konacnaListaAlarma.size();i++) {
                    if (konacnaListaAlarma.get(i).getNaslov().equals(itemList.get(position).getNaslov())) {
                        String[] dateParts = konacnaListaAlarma.get(i).getDatum().split("/");
                        if(konacnaListaAlarma.get(i).getDatum().equals(todayDate)){
                            holder.alarmTime.setText(konacnaListaAlarma.get(i).getVrijeme());
                            holder.dateDate.setText(dateParts[0] +"/"+ dateParts[1]);
        ...
        if (showShimmer) {
            ...
            holder.lokacija.setText(itemList.get(position).getLokacija());
            holder.datum.setText(dan+"/"+mjesec);
            holder.timeTime.setText(itemList.get(position).getVrijeme());
                ...
                public void onClick(View v) {
                    ...
                    for(int i=0;i<konacnaListaAlarma.size();i++) {
                        if (konacnaListaAlarma.get(i).getNaslov().toString().equals(itemList.get(position).getNaslov().toString())) {
                            ...
                            alarmIDDoSad=konacnaListaAlarma.get(i).getAlarmID();
                            positionToDelete=i;
                        }
                    }
                                ...
                                mTimePicker.setOnDismissListener(new DialogInterface.OnDismissListener() {
                                    ...
                                    public void onDismiss(DialogInterface dialog) {
                                        if(!cancelAlarmFlagAlarmLayout) {
                                            ...
                                            final RealmResults<AlarmDate> results = realm.where(AlarmDate.class).equalTo("datum", konacnaListaAlarma.get(positionToDelete).getDatum()).equalTo("vrijeme", konacnaListaAlarma.get(positionToDelete).getVrijeme()).equalTo("naslov", konacnaListaAlarma.get(positionToDelete).getNaslov()).findAll();
                                            realm.executeTransaction(new Realm.Transaction() {
                                                @Override
                                                public void execute(Realm realm) {
                                                    results.deleteAllFromRealm();
                                            ...
                                            alarmDate.setNaslov(itemList.get(position).getNaslov());
...
//*******************************************dodao prvi parametar novi datum  i zadnja dva da se ispise u notifikaciji
                                            setNotificationAlarm(danToAdd + "/" + mjesecToAdd + "/" + godinaToAdd + " " + hToAdd + ":" + minToAdd, randomID, holder.naslov.getText().toString(), holder.lokacija.getText().toString(), itemList.get(position).getDatum(), itemList.get(position).getVrijeme());
...
//*******************************************ovdje sam stavio novi datum
  ...
  //*******************************************ovo sam dodao
                                                    ...
                                                    createNotificationChannel();
//*******************************************dodao prvi parametar novi datum  i zadnja dva da se ispise u notifikaciji
                                                    setNotificationAlarm(holder.danMjeseca+"/"+holder.mjesec+"/"+holder.godina + " " + holder.alarmTime.getText().toString(), randomID, holder.naslov.getText().toString(), holder.lokacija.getText().toString(), itemList.get(position).getDatum(), itemList.get(position).getVrijeme());
                                                }
                                            } else {
                                                cancelFlag = false;
                                            }
                    ...
                    //ako je postojao alarm, mozes ga samo obrisat
                        ...
                        for(int i=0;i<konacnaListaAlarma.size();i++){
                            if( konacnaListaAlarma.get(i).getNaslov().equals(itemList.get(position).getNaslov()) && konacnaListaAlarma.get(i).getVrijeme().equals(holder.alarmTime.getText().toString())){
                                unsetNotificationAlarm(konacnaListaAlarma.get(i).getAlarmID());
                        ...
                        final RealmResults<AlarmDate> results = realm.where(AlarmDate.class).equalTo("datum",datumZaBrisanje).equalTo("vrijeme",holder.alarmTime.getText().toString()).equalTo("naslov",itemList.get(position).getNaslov()).findAll();
                        realm.executeTransaction(new Realm.Transaction() {
                            @Override
                            public void execute(Realm realm) {
                                results.deleteAllFromRealm();
...
//******************dodao da ispise u notifikaciji
    ...
    public Event getDatum(int position){
    ...
    public void shareItem(int position) {
        ...
        String opis = itemList.get(position).getOpis();
        String naslov = itemList.get(position).getNaslov();
        String vrijeme = itemList.get(position).getVrijeme();
        String lokacija = itemList.get(position).getLokacija();
        String datum = itemList.get(position).getDatum();
