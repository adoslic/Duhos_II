...
public class CalendarFragment extends Fragment implements DatePickerListener {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                ...
                for (DataSnapshot snapshot: dataSnapshot.getChildren()){
                    if(snapshot.exists()) {
                        ...
                        itemList.add(new Event(naslov,opis,datum,vrijeme,lokacija));
                ...
                for(int i=0;i<itemList.size()-1;i++){
                    for(int j=0;j<itemList.size()-i-1;j++) {
                        String date1String=itemList.get(j).getDatum()+" "+itemList.get(j).getVrijeme()+":00";
                        Date date1 = null;
                        try {
                        ...
                        Date date2 = null;
                        String date2String=itemList.get(j+1).getDatum()+" "+itemList.get(j+1).getVrijeme()+":00";
                ...
                adapter = new CalendarItemAdapter(itemList,konacnaListaAlarma,alarmVisibility);
                recyclerView.setLayoutManager(new LinearLayoutManager(getContext()));
                //recyclerView.setHasFixedSize(true);
                recyclerView.setAdapter(adapter);
                ItemTouchHelper itemTouchHelper = new ItemTouchHelper(new SwipeCalendarToShareCallback(adapter));
